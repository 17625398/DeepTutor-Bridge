...
_venv_python = None
for _venv_dirname in (".venv", "venv"):
    _candidate = (
        PROJECT_ROOT
        / _venv_dirname
...
PYTHON_EXE = os.getenv("DEEPTUTOR_PYTHON", "").strip() or (
    str(_venv_python) if _venv_python is not None else sys.executable
)
...
MESSAGES = {
    "en": {
        ...
        "installing_python_deps": "Backend dependencies missing; installing requirements/server.txt ...",
    ...
    "zh": {
        ...
        "installing_python_deps": "检测到后端依赖缺失，正在安装 requirements/server.txt ...",
...
def _resolve_integration_command(
    command: list[str],
    *,
    npm: str,
    pnpm: str | None,
    ...
    if resolved[0] == "npm":
        resolved[0] = npm
    if resolved[0] == "pnpm" and pnpm:
        resolved[0] = pnpm
    if resolved[0] == "cargo" and cargo:
        resolved[0] = cargo
    if resolved[0] == "python":
        resolved[0] = PYTHON_EXE
    return resolved


def _ensure_npm_deps(cwd: Path, *, npm: str, env: dict[str, str], language: str) -> None:
    ...
    log_info(_t(language, "starting_integration", name=f"npm:{cwd.name}:deps"))
    subprocess.run(cmd, cwd=str(cwd), env=env, check=False)


def _ensure_pnpm_deps(cwd: Path, *, pnpm: str, env: dict[str, str], language: str) -> None:
    package_json = cwd / "package.json"
    if not package_json.exists():
        return
    node_modules = cwd / "node_modules"
    ...
    lock_path = cwd / "pnpm-lock.yaml"
    cmd = [pnpm, "install"]
    if lock_path.exists():
        cmd += ["--frozen-lockfile"]

    log_info(_t(language, "starting_integration", name=f"pnpm:{cwd.name}:deps"))
    subprocess.run(cmd, cwd=str(cwd), env=env, check=False)


def _python_has_module(python_exe: str, module: str) -> bool:
    try:
...
def _ensure_python_server_deps(*, python_exe: str, language: str) -> None:
    if _python_has_module(python_exe, "uvicorn"):
        return
    req = PROJECT_ROOT / "requirements" / "server.txt"
    if not req.exists():
        return
    log_info(_t(language, "installing_python_deps"))
    subprocess.run(
...
def _probe_http(url: str) -> bool:
    try:
        with urlrequest.urlopen(url, timeout=1.2):
            return True
...
def _probe_tcp_from_url(url: str) -> bool:
    try:
        parsed = urlparse(url)
...
def main() -> None:
    ...
    backend_cmd = [PYTHON_EXE, "-m", "deeptutor.api.run_server"]
