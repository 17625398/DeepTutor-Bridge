...
## 分离部署（Web UI 与 Agent 分开）

默认情况下，Web UI 会在本地启动 Hermes agent bridge。如果需要将 Web UI 和 Hermes Agent 部署在不同的机器上，可以使用分离部署模式。

### 配置步骤

1. **在 Agent 服务器上**：确保 Hermes Agent 已启动并记录 `API_SERVER_KEY` 和网关端口（默认 8642）

2. **在 Web UI 服务器上**：创建 `.env` 文件，配置远程 Agent 地址和 API Key：

```bash
# 远程 hermes-agent 地址（gateway 端口）
HERMES_GATEWAY_URL=http://192.168.0.39:8642

# 远程 agent 的 API Key（与远程 agent config.yaml 中的 API_SERVER_KEY 一致）
...
./start.bat  # Windows
# 或
node dist/server/index.js  # 手动启动
```
...
| 变量 | 说明 |
|---|---|
| `HERMES_GATEWAY_URL` | 远程 hermes-agent 的完整 URL（含端口） |
| `HERMES_GATEWAY_API_KEY` | 远程 agent 的 API Key，必须与 agent 端的 `API_SERVER_KEY` 一致 |
