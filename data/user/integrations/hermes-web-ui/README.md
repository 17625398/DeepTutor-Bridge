...
## Separated Deployment (Web UI and Agent on Different Machines)

By default, Web UI starts a local Hermes agent bridge. If you need to deploy Web UI and Hermes Agent on different machines, use separated deployment mode.

### Configuration Steps

1. **On the Agent Server**: Ensure Hermes Agent is running and note the `API_SERVER_KEY` and gateway port (default 8642)

2. **On the Web UI Server**: Create a `.env` file with remote agent address and API Key:

```bash
# Remote hermes-agent address (gateway port)
...
Or use the pre-built production package:

```bash
# After extracting the production package
./start.bat  # Windows
# or
node dist/server/index.js  # manual start
```
...
When `HERMES_GATEWAY_URL` is set, Web UI will:
- Skip local agent bridge startup
