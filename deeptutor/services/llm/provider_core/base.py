...
try:
    ...
    _fallback_logger = logging.getLogger("deeptutor.llm")
    if not _fallback_logger.handlers:
        logging.basicConfig(level=logging.INFO)

    class _CompatLogger:
        def debug(self, msg: object, *args: object) -> None:
            _fallback_logger.debug(str(msg).format(*args) if args else str(msg))

        def info(self, msg: object, *args: object) -> None:
            _fallback_logger.info(str(msg).format(*args) if args else str(msg))

        def warning(self, msg: object, *args: object) -> None:
            _fallback_logger.warning(str(msg).format(*args) if args else str(msg))

        def error(self, msg: object, *args: object) -> None:
            _fallback_logger.error(str(msg).format(*args) if args else str(msg))

        def exception(self, msg: object, *args: object) -> None:
            _fallback_logger.exception(str(msg).format(*args) if args else str(msg))

    logger = _CompatLogger()  # type: ignore[assignment]
