...
class ToolRegistry:
    ...
    def load_plugins(self) -> None:
        try:
            from deeptutor.plugins.loader import discover_plugins, load_plugin_tool
        except Exception:
            return
        ...
        for manifest in discover_plugins():
            ...
            is_tool_entry = (
                manifest.type == "tool"
                or entry.endswith("tool.py")
                or entry.endswith("tool.py:")  # defensive
                or "tool.py:" in entry
            )
            if not entry or not is_tool_entry:
                continue
            try:
                tool = load_plugin_tool(manifest)
                if tool is None:
                    continue
                if tool.name in self._tools:
                    continue
                ...
                logger.warning("Failed to load plugin tool %s", manifest.name, exc_info=True)
